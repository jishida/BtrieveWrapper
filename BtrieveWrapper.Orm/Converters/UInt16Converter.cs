...
{
    [FieldConverter("UInt16", typeof(ushort), 2)]
    public class UInt16Converter : IFieldConverter
    {
        public object Convert(byte[] source, ushort position, ushort length, object parameter) {
            return BitConverter.ToUInt16(source, position);
        }

        public void ConvertBack(object source, byte[] destination, ushort position, ushort length, object parameter) {
            Array.Copy(BitConverter.GetBytes(System.Convert.ToUInt16(source)), 0, destination, position, length);
        }
